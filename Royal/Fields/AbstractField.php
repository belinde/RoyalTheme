	...
	public function getSearchField( &$fieldNum, $metaQuery ) {
		switch ( $this->search ) {
			case self::SEARCH_EXACT:
				$field = $this->searchFieldExact( $fieldNum, $metaQuery );
				break;
			case self::SEARCH_TEXT:
				$field = $this->searchFieldText( $fieldNum, $metaQuery );
				break;
			case self::SEARCH_RANGE:
				$field = $this->searchFieldRange( $fieldNum, $metaQuery );
				break;
			default:
				return null;
		}

		return '<div class="fieldcontainer ' . $this->search . '">' . $field . '</div>';
	}
